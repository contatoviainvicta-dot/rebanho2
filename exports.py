...
style_titulo = ParagraphStyle(
    "titulo_pec", parent=styles["Title"],
    fontSize=15, spaceAfter=4, textColor=colors.HexColor("#1F5C2E"),
)
story.append(Paragraph(_sanitizar_texto(titulo), style_titulo))
...
for sec in secoes:
    story.append(Paragraph(_sanitizar_texto(sec["titulo"]), style_sec))
    ...
    try:
        # Converter tudo para string limpa
        colunas = [_sanitizar_texto(c) for c in df.columns]
        linhas  = [[_sanitizar_texto(v) for v in row]
                   for row in df.values.tolist()]
        data = [colunas] + linhas

        # Largura adaptativa: limitar colunas largas
        n_cols = len(colunas)
        ...
        story.append(Paragraph(f"Erro ao renderizar tabela: {e}",
                               styles["Normal"]))
...
try:
    ...
    # Garantir que começa com %PDF-
    if not pdf_bytes.startswith(b"%PDF-"):
        raise ValueError("PDF gerado inválido")
    return pdf_bytes
except Exception as e:
    # Fallback: retornar texto simples formatado como pseudo-PDF
    return _pdf_fallback(titulo, secoes)
```
